 ...
 */
class Enumerator {
    private static final String TAG = "XVisio Enumerator";
    ...
    public Enumerator(Context context, DeviceListener listener) {
        ...
        if (Build.VERSION.SDK_INT >= Build.VERSION_CODES.TIRAMISU) {
            context.registerReceiver(mBroadcastReceiver, new IntentFilter(UsbUtilities.ACTION_USB_PERMISSION),RECEIVER_EXPORTED);
            context.registerReceiver(mBroadcastReceiver, new IntentFilter(UsbManager.ACTION_USB_DEVICE_ATTACHED),RECEIVER_EXPORTED);
            context.registerReceiver(mBroadcastReceiver, new IntentFilter(UsbManager.ACTION_USB_DEVICE_DETACHED),RECEIVER_EXPORTED);
    ...
    private class MessagesHandler extends Handler {
        private static final String TAG = "XVisio MessagesHandler";
            ...
            try {
                switch (msg.what) {
                    case ON_DEVICE_AVAILABLE:
                        Log.i(TAG, "handleMessage: XVisio device attached");
                        notifyOnAttach();
                        break;
                    case ON_DEVICE_UNAVAILABLE:
                        Log.i(TAG, "handleMessage: XVisio device detached");
                        notifyOnDetach();
                        break;
                }
            } catch (Exception e) {
